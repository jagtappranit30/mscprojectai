...
def generate_assessment_report(result_data: Dict[str, Any]) -> str:
    ...
    pdf.cell(
        0, 8, f"- Labour Efficiency: {result_data.get('labour_efficiency_score', 0):.1f}/50", 0, 1
    )
    pdf.cell(
        0, 8, f"- Financial Health: {result_data.get('financial_health_score', 0):.1f}/50", 0, 1
    )
    pdf.cell(
        0, 8, f"- Digital Maturity: {result_data.get('digital_maturity_score', 0):.1f}/100", 0, 1
    )
    pdf.ln(5)
    ...
    pdf.multi_cell(
        0,
        6,
        "Scores are calculated deterministically by comparing extracted financial metrics (Revenue per Employee, Margins, Liquidity) against sector-specific benchmarks using an AI-driven text extraction pipeline.",
    )
