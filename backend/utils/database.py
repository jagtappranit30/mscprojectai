...
class DatabaseService:
    ...
    # Ingestion runs
    async def create_ingestion_run(self, sector: str, company_name: str, document_type: str) -> str:
    ...
    async def update_run_status(
        self, run_id: str, status: str, confidence: float = None, error_message: str = None
    ):
        ...
        if self.enabled:
            try:
                self.client.table("ingestion_runs").update(data).eq("run_id", run_id).execute()
                return
            except Exception as e:
            ...
            rows.append(
                {
                    ...
                    "metric_value": metric_val.get("value")
                    if isinstance(metric_val, dict)
                    else metric_val,
                    "metric_unit": metric_val.get("unit") if isinstance(metric_val, dict) else None,
                    "confidence": metric_val.get("confidence", 0)
                    if isinstance(metric_val, dict)
                    else 0,
                    "source_text": metric_val.get("source", "")
                    if isinstance(metric_val, dict)
                    else "",
                }
            )
    ...
    async def get_benchmark(self, sector: str, metric_name: str) -> Dict:
        ...
        if self.enabled:
            try:
                response = (
                    self.client.table("benchmark_metrics")
                    .select("p25, p50, p75")
                    .eq("sector", sector)
        ...
        normalized_sector = (
            sector if sector in ["Manufacturing", "Services", "Retail"] else "Services"
        )
        return self.mock_benchmarks.get(
            (normalized_sector, metric_name), {"p25": 0, "p50": 0, "p75": 0}
        )
    ...
    async def store_assessment_result(self, result_data: Dict) -> str:
        ...
        if self.enabled:
            try:
                response = self.client.table("assessment_results").insert(result_data).execute()
    ...
    async def save_assessment_to_user(self, result_id: str, user_id: str):
        ...
        if self.enabled:
            try:
                self.client.table("assessment_results").update({"user_id": user_id}).eq(
                    "result_id", result_id
                ).execute()
            except Exception as e:
